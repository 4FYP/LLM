...
GOOGLE_REDIRECT_URI  = os.getenv(
    "GOOGLE_REDIRECT_URI",
    f"http://localhost:{SERVER_PORT}/auth/google/callback",
)
...
SYSTEM_WITH_ODOO = """CRITICAL — LANGUAGE RULE (highest priority, no exceptions):
Always reply in the EXACT language of the user's latest message.
English message → English reply. Arabic message → Arabic reply. Never switch based on data or history.

You are an expert Odoo ERP assistant with FULL access to a live Odoo database.
...
═══ CUSTOM MODULES — IF A RECORD/MODEL CAN'T BE FOUND ═══
Many clients have custom-built modules (e.g. their own "Purchase Requisition", approval
flows, custom reports). If 1-2 searches on the obvious standard model(s) return 0 records,
do NOT immediately tell the user "not found". Instead discover the real model first:
  odoo_search ir.model [['model','ilike','KEYWORD']] ['model','name'] limit=20
  (try the KEYWORD from the user's term, e.g. 'requisition', 'royalty', 'approval')
Then odoo_fields <found_model> to see its fields, and odoo_search/odoo_count on it directly.
Only report "not found" after trying this ir.model lookup.
NEVER assume field names — always call odoo_fields on the model first, then use only the
field names that actually exist in the response.
...
- NEVER output raw JSON, raw tool results, or a "Raw System Data" / "System Data" section.
- Present data in clean tables, bullet points, or plain sentences only.
- If user asks for raw data explicitly, show only the specific fields they asked for — still formatted, not a JSON dump.

Always confirm with user before deleting. Show created record ID after creation.
LANGUAGE RULE: Always reply in the exact language the user used in their LATEST message.
If they wrote in English → reply in English. If Arabic → Arabic. If Hindi/Gujarati → Hindi/Gujarati.
Never switch language based on data content or previous messages."""

SYSTEM_PLAIN = (
    "CRITICAL — LANGUAGE RULE (highest priority, no exceptions): "
    "Always reply in the EXACT language of the user's latest message. "
    "English message → English reply. Arabic message → Arabic reply. Never switch based on data or history. "
    "You are an expert Odoo ERP assistant. "
    "Help with modules, development, configuration, and business processes. "
    "When the user shares a file (Excel, CSV, PDF), read it carefully and answer their question "
    "with clean tables, summaries, and specific numbers — never ask them to re-share or convert the file. "
    "Always remember the file content throughout the conversation so follow-up questions are answered in context. "
    "Never output raw JSON. Format all responses with markdown tables or bullet points."
)
...
def _load_users() -> list[dict]:
    ...
    try:
        ...
        return [{"username": "admin", "password": "admin123", "name": "Admin"}]
...
def _find_or_create_google_user(email: str, name: str) -> dict:
    users = _load_users()
    user = next(
        (u for u in users if u.get("email", u["username"]).lower() == email),
        None,
    )
    if user:
        return user
...
def _google_auth_html(auth: dict) -> HTMLResponse:
    html = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"/><title>Signing in…</title></head>
<body>
<script>
  localStorage.setItem('cf_platform_token', {json.dumps(auth["token"])});
  localStorage.setItem('cf_platform_user', {json.dumps(json.dumps(auth))});
...
async def google_auth_callback(
    ...
    if error:
        return RedirectResponse(f"/signup?error={urllib.parse.quote(error)}")
    if not code or not state or state not in _oauth_states:
        return RedirectResponse("/signup?error=invalid_oauth_state")
    ...
    try:
        ...
        return RedirectResponse("/signup?error=google_auth_failed")

    email = (profile.get("email") or "").strip().lower()
    if not email:
        return RedirectResponse("/signup?error=no_email_from_google")

    name = profile.get("name") or email.split("@")[0]
    user = _find_or_create_google_user(email, name)
    return _google_auth_html(_auth_response(user))
...
@app.post("/signup")
async def platform_signup(req: SignupRequest):
    ...
    users = _load_users()
    if any(u.get("email", u["username"]).lower() == email for u in users):
...
async def chat(req: ChatRequest):
    ...
    _LEAK_MARKERS = ("<|DSML|", "tool_calls>", "invoke name=", "parameter name=")
    ...
    async def event_stream():
        ...
        tool_msgs = list(base)
        leak_retried = False
        max_iter = int(os.getenv("MAX_ITERATIONS", "25"))
            ...
            if odoo:
                for iteration in range(max_iter):
                    ...
                    if choice["finish_reason"] != "tool_calls":
                        ...
                        if content and _looks_like_leak(content):
                            if not leak_retried:
                                leak_retried = True
                                ...
                                tool_msgs.append({
                                    "role": "user",
                                    "content": (
                                        "Your last response contained malformed tool-call syntax. "
                                        "Either call a tool properly, or reply in plain natural language only — "
                                        "do not emit any XML/tag-like syntax."
                                    ),
                                })
                                continue
                            content = "I ran into trouble completing that. Please try rephrasing."
                        if content:
                ...
                tool_msgs.append({
                    "role": "user",
                    "content": (
                        "You've reached the tool call limit. Do NOT emit any tool-call syntax — "
                        "summarize what you found in plain natural language only."
                    ),
                })
                ...
                async with client.stream("POST", url, json=payload, headers=headers) as resp:
                    async for line in resp.aiter_lines():
                        ...
                        if buf_flushed:
                            ...
                            continue
                        buf += delta
                        if _looks_like_leak(buf):
                            yield chunk("I ran into trouble producing a clean answer. Please try rephrasing.")
                            buf = ""
                            buf_flushed = True
                            break
                        if len(buf) >= 60:
